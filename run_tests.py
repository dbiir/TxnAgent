...
engines = ["postgresql"]
functions = ["scalability", "hotspot-128", "skew-128", "wc_ratio-256", "wr_ratio-64", "skew-64",
             "bal_ratio-128", "wc_ratio-128", "random-128", "no_ratio-128", "pa_ratio-128",
             "wr_ratio-128", "distributed-128", "wr_ratio-64", "distributed-64", "dynamic-128", "switch-128",
             "scalability_p", "wr_ratio-128_p", "skew-128_p"]
...
def parse_args():
    ...
    parser.add_argument("-e", "--engine", dest="engine", choices=engines, type=str, required=True,
                        help="specify the workload")
    parser.add_argument("-p", "--partition", dest="pconfig_path", type=str, required=False,
                        help="specify the partition config path")
...
def get_partition_files(pfile_path: str):
    container_name = "txncompass_server"
    config_path = "/data/TxnSailsServer"
    remote_machine_ip = "worker-127"

    cmd1 = gen_docker_cmd(container_name, "find /data/TxnSailsServer/config/partition/wr" + " -type f", config_path)
...
def run_once(f: str, pconfig_path: str = ""):
    ...
    partition_file_list = []
    pfiles: bool = False
    if len(pconfig_path) > 0:
        partition_file_list = get_partition_files(pconfig_path)
        pfiles = True
    else:
        partition_file_list = ["config/partition/ycsb/partition-2.yaml"]
    ...
    for conf_file in get_config_files(f, args.engine):
        ...
        for partition_file in partition_file_list:
            if len(partition_file) == 0:
                continue
            partition_name = os.path.splitext(os.path.basename(partition_file))[0]
            result_dir = result_prefix + args.wl + "/" + f + "/" + unique_ts + "/" + partition_name + "/"
            ...
            if cc_name == "FS":
                ...
                if pfiles:
                    server_output_file_path += "/" + partition_name
                ...
                java_cmd = prefix_cmd_java_server + " -c " + config_path_local + " -s " + schema_path_local + " -d " + result_dir + case_name + " -t " + partition_file + " -p offline > " + server_output_file + " 2>&1"
            ...
            output_file_path = result_dir + case_name
            mkdir_cmd = "mkdir -p " + remote_client_dir + result_dir + case_name
            if pfiles:
                mkdir_cmd += "/" + partition_name
                output_file_path += "/" + partition_name
            ...
            if pfiles:
                client_cmd = prefix_cmd_java_client + " -b " + args.wl + " -c " + config_path + case_name + ".xml" + \
                             " --execute=true -d " + result_dir + case_name + "/" + partition_name + " > " + output_file_path + "/stdout.log"
            else:
                client_cmd = prefix_cmd_java_client + " -b " + args.wl + " -c " + config_path + case_name + ".xml" + \
                             " --execute=true -d " + result_dir + case_name + " > " + output_file_path + "/stdout.log"
            ...
            run_shell_command(remote_docker_client_cmd, 600)
...
def run_cnt(f: str, cnt: int, pconfig: str = ""):
    timestamps = []
    for i in range(cnt):
        if pconfig.strip() != "":
            ts = run_once(f, pconfig)
        else:
            ts = run_once(f)
...
if __name__ == "__main__":
    ...
    for f in ff:
        if args.pconfig_path is not None:
            pconfig_path = args.pconfig_path
            tss = run_cnt(f, args.cnt, pconfig_path)
        else:
